...
class RawEvent(models.Model):
    ...
    event_timestamp = models.DateTimeField()           # original timestamp in the event
    received_at = models.DateTimeField(default=timezone.now)  # wall-clock ingestion time
    details = models.JSONField(default=dict, blank=True)
    raw_payload = models.JSONField()                   # verbatim payload for replay

    # Dedup fingerprint: SHA256(source + user_id + event_type + event_timestamp)
...
class StudentState(models.Model):
    ...
    Each reconciliation decision creates a new version row; the latest version
    ...
    # Timeline: ordered list of reconciled event summaries (JSON)
    timeline = models.JSONField(default=list)

    # Triggering event(s) that caused this version
...
class AuditLog(models.Model):
    """
    One audit record for every reconciliation decision made.
    """
    id = models.AutoField(primary_key=True)
    user_id = models.CharField(max_length=100, db_index=True)
